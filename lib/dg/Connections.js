...
async function start(e, serverUrl, qq, newId = false) {
  ...
  for (let i = 0; i < 10; i++) {
    if (Connections.connections[qq].clientId) break
    if (i == 9) return false
    await sleep(1000)
  }
  let Qr_url = `https://www.dungeon-lab.com/app-download.php#DGLAB-SOCKET#${serverUrl}${Connections.connections[qq].clientId}`
  let isConsole = console.__proto__ == logger.__proto__
  try {
    const buffer = await 生成二维码(Qr_url)
    let msgid
    if (isConsole) {
      const fs = await import("fs")
      const filePath = path.join(process.cwd(), "qrcode.png")
      fs.writeFileSync(filePath, buffer)
      console.log(`二维码已保存至 ${filePath}`)
    } else {
      msgid = await e.reply([
    ...
    let cs = 20
    for (let n = 0; n < cs; n++) {
      if (Connections.connections[qq].检查ID状态()) {
        Connections.connections[qq].buffer = buffer
        e.reply("连接完成")
        if (!isConsole)
          e.isGroup ? e.group.recallMsg(msgid.message_id) : e.friend.recallMsg(msgid.message_id)
        e.reply(
          `用户${qq}的设备\n状态: ${Connections.connections[qq].clientId != "" ? "已连接" : "连接已断开"}\n设备状态: ${Connections.connections[qq].targetId != "" ? "设备已连接" : "设备连接已断开"}\nA通道强度大小: ${Connections.connections[qq].A}\nB通道强度大小: ${Connections.connections[qq].B}\nA通道强度大小上限: ${Connections.connections[qq].A_S}\nB通道强度大小上限: ${Connections.connections[qq].B_S}`,
        )
      ...
      if (n === cs - 1) {
        e.reply("连接失败")
        e.isGroup ? e.group.recallMsg(msgid.message_id) : e.friend.recallMsg(msgid.message_id)
        Connections.connections[qq].断开连接()
        delete Connections.connections[qq]
        return false
      }
    }
  } catch (err) {
...
class Connection {
  ...
  emit(event, ...args) {
    this.listener.forEach(callback => {
      if (callback.event == event) callback(...args)
    })
  }
  检查ID状态(id = "targetId") {
